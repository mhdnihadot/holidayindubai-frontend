...
const SearchBar: React.FC = () => {
  ...
  return (
    <div className="w-full flex justify-center py-6 bg-white">
      <div
        ref={containerRef}
        className={`bg-white rounded-full p-2 flex items-center shadow-xs border border-gray-100 mx-4 overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] ${isExpanded ? 'max-w-3xl w-full' : 'max-w-4xl w-full'}`}
      >

        {/* Location Section */}
        <div
          onClick={handleExpand}
          className={`flex items-center px-6 py-2 gap-4 cursor-pointer  rounded-full transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] ${isExpanded ? 'flex-[1_0_0%] min-w-0 bg-white shadow-sm ring-1 ring-gray-200' : 'flex-1 min-w-[150px]'}`}
        >
          ...
          <div className="relative flex-1 h-10 flex items-center overflow-hidden">
            ...
            <div className={`absolute inset-0 flex flex-col justify-center whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] ${isExpanded ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
              <span className="text-black font-semibold text-[15px] ">Location</span>
              <span className="text-gray-500 text-sm">Search city or area</span>
            </div>
        ...
        {/* Category Section */}
        <div className={`flex items-center cursor-pointer hover:bg-gray-100/50 rounded-full transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] ${isExpanded ? 'px-4 py-2 flex-none' : 'flex-1 px-6 py-2 gap-4'}`}>
          ...
          <div className={`flex flex-col overflow-hidden whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] ${isExpanded ? 'max-w-0 opacity-0' : 'max-w-[150px] opacity-100'}`}>
            <span className="text-black font-semibold text-[15px] ">Category</span>
        ...
        {/* Vibe Section */}
        <div className={`flex items-center cursor-pointer hover:bg-gray-100/50 rounded-full transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] overflow-hidden ${isExpanded ? 'max-w-0 opacity-0 px-0' : 'flex-1 px-6 py-2 gap-4'}`}>
          <Sparkles className="w-6 h-6 text-black shrink-0" strokeWidth={0.8} />
          <div className="flex flex-col whitespace-nowrap">
            <span className="text-black font-semibold text-[15px] ">Vibe</span>
            <span className="text-gray-500 text-sm">Adventure, Family..</span>
          </div>
        </div>
        ...
        <button
          ...
          className={`bg-[#FF1645] hover:bg-[#f80d3c] text-white rounded-full flex items-center justify-center transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] flex-shrink-0 ml-2 mr-1 ${isExpanded ? 'w-14 h-14' : 'w-14 h-14'}`}
        >
